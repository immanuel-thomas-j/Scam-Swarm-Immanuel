...
// ── API Adapter — swap this one function to switch back to Gemini ─────────────
...
export default function Analyzer() {
  ...
  const executeSwarmAnalysis = async () => {
    ...
    if (!import.meta.env.VITE_GROQ_API_KEY) { setErrorMsg('⚠ VITE_GROQ_API_KEY is not configured. Add it to your .env file.'); return; }
...
{"overallRiskScore":<0-100>,"verdict":"SAFE or SUSPICIOUS or CRITICAL THREAT","scamCategory":"<category>","executiveSummary":"<2 sentence analysis>","confidence":<0-100>,"threatIndicators":["<phrase1>","<phrase2>","<phrase3>"],"recommendedActions":["<action1>","<action2>","<action3>"],"agents":[{"name":"Link & Domain Investigator","status":"SAFE or SUSPICIOUS or MALICIOUS","finding":"<finding>"},{"name":"Psychological Urgency Cop","status":"SAFE or SUSPICIOUS or MALICIOUS","finding":"<finding>"},{"name":"Financial Pattern Auditor","status":"SAFE or SUSPICIOUS or MALICIOUS","finding":"<finding>"}]}`;
    ...
    try {
      ...
      if (msg.includes('429') || msg.includes('rate_limit')) setErrorMsg('⏳ Rate limit hit. Wait ~30 seconds and try again.');
      else if (msg.includes('401') || msg.includes('invalid_api_key')) setErrorMsg('🔑 Invalid API key. Check your VITE_GROQ_API_KEY in the .env file.');
      else setErrorMsg('❌ Analysis failed. Check your API key and network connection.');
    } finally {
  ...
  return (
    <div className="app-root">
      ...
      <footer className="app-footer">
        ...
        <span>Powered by <strong style={{ color: '#22d3ee' }}>LLaMA 3.3 · Groq</strong></span>
        <span className="footer-sep">//</span>
