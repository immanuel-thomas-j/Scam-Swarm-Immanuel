...
// ── Mock Threat Intelligence Database (OSINT Feed) ───────────────────────────
const THREAT_INTEL_DB = {
  reportedVPAs: ['claim-prize@ybl', 'upi-verify-sbi@okhdfcbank', 'bonus-claim@paytm', 'secure-sbi@ybl'],
  reportedDomains: ['sbi-verification-secure-portal.net', 'netflix-billing-update.com', 'google-india-draw.xyz', 'icici-security-portal.in']
};
...
function runLocalHeuristicAnalysis(text) {
  ...
  if (urls.length > 0) {
    ...
    urls.forEach((urlStr) => {
      try {
        ...
        if (THREAT_INTEL_DB.reportedDomains.includes(hostname)) {
          findings.push({
            type: 'KNOWN_SCAM_DOMAIN',
            label: 'Blacklisted Scam Domain',
            desc: `Domain '${hostname}' matches known reported phishing campaigns in active threat intelligence databases.`,
            severity: 'CRITICAL',
          });
          scoreWeight += 45;
        }
  ...
  if (upis.length > 0) {
    upis.forEach((upi) => {
      ...
      // Blacklist VPA check
      if (THREAT_INTEL_DB.reportedVPAs.includes(upiLower)) {
        findings.push({
          type: 'KNOWN_SCAM_VPA',
          label: 'Blacklisted UPI Address',
          desc: `UPI handle '${upi}' matches reported fraudulent payment vectors.`,
          severity: 'CRITICAL',
        });
        scoreWeight += 45;
      }
...
function RecommendedActions({ verdict, aiActions = [] }) {
  ...
  return (
    <section className="panel actions-panel-wrapper animate-slide-up" style={{ borderColor: `${color}30` }}>
      <div className="panel-header" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <ShieldIcon size={20} color="var(--cyan)" />
        <h2 className="panel-title">Recommended Response Actions</h2>
        {verdict === 'CRITICAL THREAT' ? (
          <div className="panel-badge" style={{ borderColor: 'rgba(239,68,68,0.3)', color: 'var(--red)', background: 'var(--red-glow)' }}>URGENT</div>
        ) : (
          <div className="panel-badge">RESPONSE</div>
        )}
      </div>
...
export default function Analyzer() {
  ...
  const copyMarkdownReport = () => {
    ...
    const heuristicsText = swarmPayload.heuristics?.findings?.length
      ? swarmPayload.heuristics.findings.map(f => `* **[${f.severity}]** ${f.label}: *${f.desc}*`).join('\n')
      : '* No local threat heuristics matched.';
      
    const md = `
# 🛡️ Scam Swarm Analysis Report
---
* **Date of Scan**: ${new Date().toISOString().slice(0, 10)}
* **Overall Risk Score**: ${swarmPayload.overallRiskScore} / 100
* **Composite Threat Verdict**: **${swarmPayload.verdict}**
* **Target Classification**: \`${swarmPayload.scamCategory}\`
* **Confidence Level**: ${swarmPayload.confidence}%

## 📋 Executive Cognitive Summary
> ${swarmPayload.executiveSummary}

## 🔍 Deterministic Heuristic Diagnostics
${heuristicsText}

## 📊 Threat Metrics
* **Linguistic Urgency**: ${swarmPayload.metrics?.linguisticUrgency ?? 0}%
* **Domain & Infrastructure Risk**: ${swarmPayload.metrics?.domainRisk ?? 0}%
* **Financial Manipulation Weight**: ${swarmPayload.metrics?.financialRisk ?? 0}%

## 🤖 Granular Agent Telemetry
${swarmPayload.agents.map(a => `### • ${a.name} (Status: **${a.status}**)
> *${a.finding}*`).join('\n\n')}

---
*Generated by Scam Swarm Console (Helpline: 1930 | cybercrime.gov.in)*
    `.trim();
    ...
    <div className="app-root">
      ...
      <main className="app-main">
        ...
        <div className="app-column">
          ...
          {swarmPayload && swarmPayload.heuristics && swarmPayload.heuristics.findings?.length > 0 && (
            <section className="panel heuristics-panel-wrapper animate-slide-up" style={{ animationDelay: '50ms' }}>
              <div className="panel-header" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <ShieldIcon size={20} color="var(--cyan)" />
                <h2 className="panel-title">Deterministic Heuristic Diagnostics</h2>
                <div className="panel-badge">LOCAL ENGINE</div>
              </div>
          ...
          <section className="panel analytics-panel" ref={dashboardRef}>
            ...
            {swarmPayload && (
              <div className="results-container animate-flicker">
                ...
                {swarmPayload.metrics && (
                  <div className="heuristics-panel animate-slide-up" style={{ animationDelay: '80ms', borderColor: 'var(--border-dim)' }}>
                    <div className="block-header" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <AnalyticsIcon size={16} color="var(--cyan)" />
                      <span className="block-label">THREAT VECTOR METRIC TRIAGE</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', padding: '6px 0' }}>
                      <div className="metric-row">
                        <div className="metric-meta" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <span style={{ fontSize: '12px', fontWeight: '500', color: 'var(--text-secondary)' }}>Linguistic Urgency Score</span>
                          <span style={{ fontSize: '12px', fontWeight: '700', color: swarmPayload.metrics.linguisticUrgency >= 70 ? 'var(--red)' : swarmPayload.metrics.linguisticUrgency >= 45 ? 'var(--amber)' : 'var(--green)' }}>{swarmPayload.metrics.linguisticUrgency}%</span>
                        </div>
                        <div className="bar-track" style={{ height: '6px', background: 'var(--bg-deep)', borderRadius: '3px', overflow: 'hidden' }}>
                          <div className="bar-fill" style={{ width: `${swarmPayload.metrics.linguisticUrgency}%`, height: '100%', backgroundColor: swarmPayload.metrics.linguisticUrgency >= 70 ? 'var(--red)' : swarmPayload.metrics.linguisticUrgency >= 45 ? 'var(--amber)' : 'var(--green)' }}/>
                        </div>
                      </div>
                      <div className="metric-row">
                        <div className="metric-meta" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <span style={{ fontSize: '12px', fontWeight: '500', color: 'var(--text-secondary)' }}>Domain &amp; Infrastructure Risk</span>
                          <span style={{ fontSize: '12px', fontWeight: '700', color: swarmPayload.metrics.domainRisk >= 70 ? 'var(--red)' : swarmPayload.metrics.domainRisk >= 45 ? 'var(--amber)' : 'var(--green)' }}>{swarmPayload.metrics.domainRisk}%</span>
                        </div>
                        <div className="bar-track" style={{ height: '6px', background: 'var(--bg-deep)', borderRadius: '3px', overflow: 'hidden' }}>
                          <div className="bar-fill" style={{ width: `${swarmPayload.metrics.domainRisk}%`, height: '100%', backgroundColor: swarmPayload.metrics.domainRisk >= 70 ? 'var(--red)' : swarmPayload.metrics.domainRisk >= 45 ? 'var(--amber)' : 'var(--green)' }}/>
                        </div>
                      </div>
                      <div className="metric-row">
                        <div className="metric-meta" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <span style={{ fontSize: '12px', fontWeight: '500', color: 'var(--text-secondary)' }}>Financial Manipulation Weight</span>
                          <span style={{ fontSize: '12px', fontWeight: '700', color: swarmPayload.metrics.financialRisk >= 70 ? 'var(--red)' : swarmPayload.metrics.financialRisk >= 45 ? 'var(--amber)' : 'var(--green)' }}>{swarmPayload.metrics.financialRisk}%</span>
                        </div>
                        <div className="bar-track" style={{ height: '6px', background: 'var(--bg-deep)', borderRadius: '3px', overflow: 'hidden' }}>
                          <div className="bar-fill" style={{ width: `${swarmPayload.metrics.financialRisk}%`, height: '100%', backgroundColor: swarmPayload.metrics.financialRisk >= 70 ? 'var(--red)' : swarmPayload.metrics.financialRisk >= 45 ? 'var(--amber)' : 'var(--green)' }}/>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="export-bar">
                  <button className="export-btn" onClick={copyReport} style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center' }}>
                    {copied ? '✓ Text Copied!' : <><ClipboardIcon size={14} /> Copy Text</>}
                  </button>
                  <button className="export-btn" onClick={copyMarkdownReport} style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center' }}>
                    {copiedMD ? '✓ Markdown Copied!' : <><ClipboardIcon size={14} /> Copy Markdown</>}
                  </button>
